...
#y_pred3 = np.select([y_pred <= .2, y_pred>.8], [np.zeros_like(y_pred), np.ones_like(y_pred)])
#y_pred = (y_pred > .5).astype(int)
#sns.scatterplot(y, y_pred, marker="+")
"""plt.scatter(x = y, y = y_pred2)
plt.xlabel('Y Test')
plt.ylabel('Predicted Y')"""
...
ax.set_title('Resíduos vs Valores Preditos Predicted Values', fontsize=16)
